var width = 960, height = 800, radius = Math.min(width, height) / 1.9, spacing = .09;

			var formatSecond = d3.time.format("%-S seconds"), formatMinute = d3.time
					.format("%-M minutes"), formatHour = d3.time
					.format("%-H hours"), formatDay = d3.time.format("%A"), formatDate = function(
					d) {
				d = d.getDate();
			...
			var color = d3.scale.linear().range(
					[ "hsl(-180,60%,50%)", "hsl(180,60%,50%)" ]).interpolate(
					function(a, b) {
			...
			var arcBody = d3.svg.arc().startAngle(0).endAngle(function(d) {
			...
			var arcCenter = d3.svg.arc().startAngle(0).endAngle(function(d) {
				return d.value * 2 * Math.PI;
			}).innerRadius(function(d) {
			...
			field.append("text").attr("dy", ".35em").attr("dx", ".75em").style(
					"text-anchor", "start").append("textPath").attr(
					"startOffset", "50%").attr("class", "arc-text").attr(
			...
			function tick() {
				if (!document.hidden)
					...
					}).transition().ease("elastic").duration(500).each(
							fieldTransition);
			...
			function fieldTransition() {
				...
				field.select(".arc-text").text(function(d) {
					return d.text;
				});
			}
